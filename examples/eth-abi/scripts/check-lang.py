...
if __name__ == "__main__":
    ...
    if files_with_chinese:
        ...
        for file_path, line_number, line in files_with_chinese:
            # Construct VSCode clickable link
            vscode_link = f"file://{os.path.abspath(file_path)}:{line_number}"
            # print(f"{vscode_link} (Line {line_number}): {line}")
            print(f"{vscode_link} (Line {line_number})")
